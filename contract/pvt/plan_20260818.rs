...
fn return_to_parent_plan(parent_plan_chunk: &mut VestingPlanChunk, plans: &[YearlyPlan]) -> Result<()> {
    ...
    if is_early_bird {
        ...
        // PUA-47: return은 소진된(amount==0) unreleased 슬롯에도 용량을 되돌려야 함
        // (차감용 non-zero 후보 helper를 재사용하면 원래 소비 트랜치가 skip됨)
        let child_unreleased: Vec<&YearlyPlan> =
            plans.iter().filter(|p| !p.released).collect();
        let parent_indices =
            different_tge_parent_return_indices(&parent_plan_chunk.plans)?;
...
/// PUA-47: different-TGE **반환** 후보 (return_to_parent_plan).
/// TGE(첫 unreleased)만 skip하고, amount==0 unreleased 슬롯도 포함해
/// 원래 소비된 트랜치로 capacity를 복원한다.
fn different_tge_parent_return_indices(plans: &[YearlyPlan]) -> Result<Vec<usize>> {
    let unreleased: Vec<usize> = plans
        .iter()
        .enumerate()
        .filter(|(_, p)| !p.released)
    ...
    Ok(unreleased.into_iter().skip(1).collect())
}

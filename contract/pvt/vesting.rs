...
declare_id!("HszBeEABL4oKR92hxqQKJJDuqYuVBCCqQhsTAZmG1zth");
...
pub mod vesting_test {
    ...
    pub fn update_vesting_info(ctx: Context<UpdateVestingInfo>, params: UpdateVestingParams) -> Result<()> {
        let vesting_account = &mut ctx.accounts.vesting_account;
        ...
        if let Some(release_amount) = params.release_amount {
            require!(release_amount <= vesting_account.total_amount, VestingError::InvalidParameters);
...
// 기존 yearly_amounts 기반 계산 로직 주석 처리
/*
fn calculate_releasable_amount(vesting: &VestingAccount, current_time: i64) -> u64 {
    if !vesting.is_active || current_time < vesting.cliff_time {
        return 0;
    }

    // 1년을 초 단위로 계산 (대략적인 값)
    let seconds_in_year = 365 * 24 * 3600;
    let years_passed = ((current_time - vesting.start_time) / seconds_in_year as i64) as usize;

    let mut vested_total: f64 = 0.0;
    for (i, &amount) in vesting.yearly_amounts.iter().enumerate() {
        if i < years_passed {
            vested_total += amount;
        }
    }

    let already_released = vesting.released_amount as f64;
    if vested_total <= already_released {
        0
    } else {
        (vested_total - already_released).round() as u64
    }
}

fn calculate_next_release_time(vesting: &VestingAccount, current_time: i64) -> i64 {
    let seconds_in_year = 365 * 24 * 3600;
    if current_time < vesting.start_time + seconds_in_year {
        return vesting.start_time + seconds_in_year;
    }
    let years_passed = ((current_time - vesting.start_time) / seconds_in_year as i64) as usize;
    if years_passed >= vesting.yearly_amounts.len() {
        vesting.end_time
    } else {
        vesting.start_time + ((years_passed + 1) as i64) * seconds_in_year as i64
    }
}
*/
...
pub struct CreateVesting<'info> {
    ...
    // beneficiary.key().as_ref() ->  admin_token_account.key().as_ref()
    #[account(
        init_if_needed,
        payer = admin,
        token::mint = token_mint,

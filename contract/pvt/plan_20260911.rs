...
use anchor_lang::Discriminator;                       // VestingPlanChunk::DISCRIMINATOR (PUA-50 migrate parse)
use anchor_lang::system_program::{self, Transfer as SystemTransfer}; // PUA-50 rent top-up CPI
...
/// YearlyPlan.parent_plan_index sentinel — parent 미바인딩(top-level) 또는 미기록
const PARENT_PLAN_INDEX_UNSET: u8 = u8::MAX;
const OLD_YEARLY_PLAN_SIZE: usize = 8 + 8 + 1; // release_time + amount + released (pre-PUA-47)
const NEW_YEARLY_PLAN_SIZE: usize = 8 + 8 + 1 + 1; // + parent_plan_index
/// 구 VestingPlanChunk 고정 space (마이그레이션 전)
const OLD_PLAN_CHUNK_SPACE: usize = 8 + 32 + 4 + (MAX_PLANS * OLD_YEARLY_PLAN_SIZE);
/// plan chunk 계정 space: discriminator + vesting pubkey + vec len + plans
/// YearlyPlan = release_time(8) + amount(8) + released(1) + parent_plan_index(1)
const PLAN_CHUNK_SPACE: usize = 8 + 32 + 4 + (MAX_PLANS * NEW_YEARLY_PLAN_SIZE);
...
pub mod vesting {                                      // Start of the Anchor program module
    ...
    pub fn do_vesting(
        ...
        // PUA-07 / PUA-14: destination은 beneficiary ATA만 허용 (mint-wallet routing 제거)
    ...
    pub fn user_create_vesting(
        ...
        {
            let parent = &mut ctx.accounts.parent_vesting_account;
            ...
            assert_vesting_plan_cap(
                parent.total_amount,
                parent.released_amount,
                &ctx.accounts.parent_plan_chunk.plans,
            )?;
        }
    ...
    pub fn append_yearly_plan(
        ...
        let plans = if deduct {
            ...
            // PUA-51: 기존 unreleased child 바인딩과 겹치지 않도록 existing plans 전달
            deduct_parent_for_append(
                parent_chunk,
                &ctx.accounts.plan_chunk.plans,
                child_tge_time,
                append_amount,
                plans,
            )?
        ...
        // - top-level: lockup(parent_vault) → beneficiary vault (cap slack 확보)
        {
            let admin_key = ctx.accounts.admin.key();
            let token_vault_key = ctx.accounts.token_vault.key();
            let (_vault_auth, vault_auth_bump) = Pubkey::find_program_address(
    ...
    /// PUA-50: 구 YearlyPlan(17B) VestingPlanChunk → 신 레이아웃(18B + parent_plan_index) 일회성 마이그레이션
    /// `parent_plan_indices`: top-level은 빈 vec.
    /// child same-TGE는 빈 vec면 release_time으로 자동 복원, 아니면 plans.len()과 동일해야 함.
    /// child different-TGE는 plans.len()과 동일한 명시적 parent index(검증됨) 필수.
    pub fn migrate_plan_chunk(
        ctx: Context<MigratePlanChunk>,
        parent_plan_indices: Vec<u8>,
    ) -> Result<()> {
        require!(
            ctx.accounts.admin_config.admin == ctx.accounts.admin.key(),
            VestingError::Unauthorized
        ...
        let plan_chunk_info = ctx.accounts.plan_chunk.to_account_info();
        require_keys_eq!(
            *plan_chunk_info.owner,
            *ctx.program_id,
            VestingError::Unauthorized
        );

        // 이미 새 space면 신 레이아웃으로 로드 가능한지 확인하고 no-op
        if plan_chunk_info.data_len() >= PLAN_CHUNK_SPACE {
            let data = plan_chunk_info.try_borrow_data()?;
            let mut slice: &[u8] = &data;
            let chunk = VestingPlanChunk::try_deserialize(&mut slice)
                .map_err(|_| error!(VestingError::InvalidParameters))?;
            require_keys_eq!(
                chunk.vesting_account,
                ctx.accounts.vesting_account.key(),
                VestingError::InvalidParameters
            );
            return Ok(());
        ...
        require!(
            plan_chunk_info.data_len() == OLD_PLAN_CHUNK_SPACE,
            VestingError::InvalidParameters
        );

        let (vesting_key, mut migrated_plans) = {
            let data = plan_chunk_info.try_borrow_data()?;
            parse_old_vesting_plan_chunk(&data)?
        };
        require_keys_eq!(
            vesting_key,
            ctx.accounts.vesting_account.key(),
        ...
        let is_child = ctx.accounts.vesting_account.token_vault
            != ctx.accounts.vesting_account.parent_vault;
        if is_child {
            let parent_vesting = ctx
                .accounts
                .parent_vesting_account
                .as_ref()
                .ok_or(VestingError::InvalidParentPlan)?;
            let parent_chunk_ai = ctx
                .accounts
                .parent_plan_chunk
                .as_ref()
                .ok_or(VestingError::InvalidParentPlan)?;
            // parent는 반드시 신 레이아웃(먼저 migrate)
            require!(
                parent_chunk_ai.data_len() >= PLAN_CHUNK_SPACE,
                VestingError::InvalidParameters
            );
            require_keys_eq!(
                *parent_chunk_ai.owner,
                *ctx.program_id,
                VestingError::Unauthorized
            );
            let expected_parent_plans = Pubkey::find_program_address(
                &[b"plans", parent_vesting.key().as_ref()],
                ctx.program_id,
            )
            .0;
            require_keys_eq!(
                parent_chunk_ai.key(),
                expected_parent_plans,
                VestingError::InvalidParameters
            );

            let parent_data = parent_chunk_ai.try_borrow_data()?;
            let mut slice: &[u8] = &parent_data;
            let parent_chunk = VestingPlanChunk::try_deserialize(&mut slice)
                .map_err(|_| error!(VestingError::InvalidParameters))?;
            require_keys_eq!(
            ...
            let is_valid_parent_vault = ctx.accounts.vesting_account.parent_vault
                == parent_vesting.token_vault
                || ctx.accounts.vesting_account.parent_vault == parent_vesting.beneficiary_vault;
            ...
            apply_parent_plan_indices_on_migrate(
                &mut migrated_plans,
                &parent_chunk.plans,
                &parent_plan_indices,
            )?;
            ...
            require!(
                parent_plan_indices.is_empty(),
                VestingError::InvalidParameters
            );
            require!(
                ctx.accounts.parent_vesting_account.is_none()
                    && ctx.accounts.parent_plan_chunk.is_none(),
                VestingError::InvalidParameters
            );
        }

        // rent top-up: System Program transfer CPI (raw lamport mutate 금지)
        let rent = Rent::get()?;
        let new_minimum = rent.minimum_balance(PLAN_CHUNK_SPACE);
        let current_lamports = plan_chunk_info.lamports();
        if current_lamports < new_minimum {
            let diff = new_minimum
        ...
        plan_chunk_info.realloc(PLAN_CHUNK_SPACE, false)?;

        let new_chunk = VestingPlanChunk {
            vesting_account: vesting_key,
            plans: migrated_plans,
        };
        {
            let mut data = plan_chunk_info.try_borrow_mut_data()?;
            data.fill(0);
...
fn ensure_released_plans_immutable(
    ...
    for old in existing.iter().filter(|p| p.released) {
        ...
        require!(
            matching.parent_plan_index == old.parent_plan_index,
            VestingError::InvalidParameters
        );
    }
    ...
    for new_plan in updated.iter().filter(|p| p.released) {
        ...
        require!(
            matching.parent_plan_index == new_plan.parent_plan_index,
            VestingError::InvalidParameters
        );
    }
...
fn assert_parent_chunk_binding(
    ...
    parent_chunk: &Account<MigratedPlanChunk>,
) -> Result<()> {
    require_keys_eq!(
        parent_chunk.vesting_account,
        parent_vesting.key(),
...
/// PUA-50: 구 17바이트 YearlyPlan 레이아웃으로 VestingPlanChunk raw 파싱
fn parse_old_vesting_plan_chunk(data: &[u8]) -> Result<(Pubkey, Vec<YearlyPlan>)> {
    require!(
        data.len() >= 8 + 32 + 4,
        VestingError::InvalidParameters
    );
    require!(
        data.len() == OLD_PLAN_CHUNK_SPACE,
        VestingError::InvalidParameters
    );

    let disc = VestingPlanChunk::DISCRIMINATOR;
    require!(data[0..8] == disc, VestingError::InvalidParameters);

    let vesting_bytes: [u8; 32] = data[8..40]
        .try_into()
        .map_err(|_| error!(VestingError::InvalidParameters))?;
    let vesting_account = Pubkey::new_from_array(vesting_bytes);
    let len_bytes: [u8; 4] = data[40..44]
        .try_into()
        .map_err(|_| error!(VestingError::InvalidParameters))?;
    let n = u32::from_le_bytes(len_bytes) as usize;
    require!(n <= MAX_PLANS, VestingError::InsufficientSpace);

    let need = 44usize
        .checked_add(
            n.checked_mul(OLD_YEARLY_PLAN_SIZE)
                .ok_or(VestingError::Overflow)?,
        )
        .ok_or(VestingError::Overflow)?;
    require!(data.len() >= need, VestingError::InvalidParameters);

    let mut offset = 44usize;
    let mut plans = Vec::with_capacity(n);
    for _ in 0..n {
        let rt_bytes: [u8; 8] = data[offset..offset + 8]
            .try_into()
            .map_err(|_| error!(VestingError::InvalidParameters))?;
        let release_time = i64::from_le_bytes(rt_bytes);
        offset += 8;
        let amt_bytes: [u8; 8] = data[offset..offset + 8]
            .try_into()
            .map_err(|_| error!(VestingError::InvalidParameters))?;
        let amount = u64::from_le_bytes(amt_bytes);
        offset += 8;
        let released = data[offset] != 0;
        offset += 1;
        ...
        plans.push(YearlyPlan {
            ...
            amount,
            released,
            parent_plan_index: PARENT_PLAN_INDEX_UNSET,
        });
    }

    Ok((vesting_account, plans))
}

/// PUA-50: migrate 시 parent_plan_index 복원.
/// same-TGE + 빈 indices → release_time 매칭.
/// different-TGE → 호출자 제공 index 검증 후 기록(추정 금지).
/// CertiK 09/07: 전부 released면 UNSET short-circuit; unreleased는 parent tranche released여도 역사 index 허용.
fn apply_parent_plan_indices_on_migrate(
    plans: &mut [YearlyPlan],
    parent_plans: &[YearlyPlan],
    parent_plan_indices: &[u8],
) -> Result<()> {
    // 활성 parent binding이 없으면 identity 복원 불필요 (parent unreleased≥2 게이트 회피)
    if plans.iter().all(|p| p.released) {
        for plan in plans.iter_mut() {
            plan.parent_plan_index = PARENT_PLAN_INDEX_UNSET;
        }
        return Ok(());
    ...
    let parent_tge_time = parent_plans.first().map(|p| p.release_time);
    let same_tge = child_tge_time == parent_tge_time;

    if same_tge && parent_plan_indices.is_empty() {
        for plan in plans.iter_mut() {
            if plan.released {
                plan.parent_plan_index = PARENT_PLAN_INDEX_UNSET;
                continue;
            }
            let parent_idx = parent_plans
                .iter()
                .position(|p| p.release_time == plan.release_time)
                .ok_or(VestingError::InvalidParameters)?;
            // parent tranche가 이미 released여도 역사 identity는 기록 (return은 별도 AlreadyReleased)
            plan.parent_plan_index =
    ...
    require!(
        parent_plan_indices.len() == plans.len(),
        VestingError::InvalidParameters
    );

    let mut used_diff: Vec<u8> = Vec::new();
    for (plan, &idx) in plans.iter_mut().zip(parent_plan_indices.iter()) {
        if plan.released {
            require!(
                idx == PARENT_PLAN_INDEX_UNSET,
                VestingError::InvalidParameters
            );
            plan.parent_plan_index = PARENT_PLAN_INDEX_UNSET;
            continue;
        ...
        require!(idx != PARENT_PLAN_INDEX_UNSET, VestingError::InvalidParameters);
        let parent_idx = idx as usize;
        require!(
            parent_idx < parent_plans.len(),
            VestingError::InvalidParameters
        );
        // 역사 parent index 복원: parent tranche released 여부는 migrate에서 요구하지 않음
        // (released parent로 capacity return 금지는 return_to_parent_plan 유지)

        if same_tge {
            require!(
                parent_plans[parent_idx].release_time == plan.release_time,
                VestingError::InvalidParameters
            );
        } else {
            // different-TGE: 첫 plan(TGE) 슬롯 제외한 역사 바인딩만 허용 (released 포함)
            require!(parent_idx != 0, VestingError::InvalidParameters);
            require!(!used_diff.contains(&idx), VestingError::InvalidParameters);
            used_diff.push(idx);
        }

        plan.parent_plan_index = idx;
    }
...
fn deduct_parent_for_append(
    ...
    existing_child_plans: &[YearlyPlan],
    ...
    if tge_equal {
        ...
        // PUA-51: child의 기존 unreleased 바인딩이 점유한 parent index는 append 후보에서 제외
        // (중복 parent_plan_index → 이후 same-size update의 take 실패 방지)
        let occupied_parent_indices: Vec<usize> = existing_child_plans
            .iter()
            .filter(|p| !p.released && p.parent_plan_index != PARENT_PLAN_INDEX_UNSET)
            .map(|p| p.parent_plan_index as usize)
            .collect();

        let parent_candidate_indices: Vec<usize> =
            different_tge_parent_candidate_indices(&parent_chunk.plans)?
                .into_iter()
                .filter(|idx| !occupied_parent_indices.contains(idx))
                .collect();
...
fn take_from_parent_plan(
    ...
    if is_early_bird {
        ...
        for i in 0..plans.len() {
            require!(
                plans[i].release_time == parent_plan_chunk.plans[i].release_time,
                VestingError::InvalidParameters
            );
            if plans[i].released {
                continue;
            }
            require!(!parent_plan_chunk.plans[i].released, VestingError::AlreadyReleased);
            require!(
                parent_plan_chunk.plans[i].amount >= plans[i].amount,
                VestingError::InsufficientAmount
            );

            parent_plan_chunk.plans[i].amount = parent_plan_chunk.plans[i]
                .amount
                .checked_sub(plans[i].amount)
                .ok_or(VestingError::Overflow)?;
            // same-TGE도 identity 기록 (반환/감사 일관성)
            plans[i].parent_plan_index = u8::try_from(i).map_err(|_| VestingError::InvalidParameters)?;
        }
    } else {
        // different-TGE: TGE(첫 unreleased) skip 후 amount > 0 후보만 차감 (PUA-34)
        // 소비한 parent index를 child에 기록 (PUA-47)
        let child_indices: Vec<usize> = plans
            .iter()
            .enumerate()
        ...
        require!(
            parent_indices.len() >= child_indices.len(),
            VestingError::InsufficientAmount
        );

        for (&child_idx, &parent_idx) in child_indices.iter().zip(parent_indices.iter()) {
            let amount = plans[child_idx].amount;
            let parent_plan = &mut parent_plan_chunk.plans[parent_idx];
            require!(
                parent_plan.amount >= amount,
                VestingError::InsufficientAmount
            );
            parent_plan.amount = parent_plan
                ...
                .checked_sub(amount)
                .ok_or(VestingError::Overflow)?;
            plans[child_idx].parent_plan_index =
                u8::try_from(parent_idx).map_err(|_| VestingError::InvalidParameters)?;
        }
    }
    ...
    if is_early_bird {
        ...
        for (child_plan, parent_plan) in plans.iter().zip(parent_plan_chunk.plans.iter_mut()) {
            ...
            if !child_plan.released {
                // 이미 해지된 parent tranche에는 용량을 되돌리지 않음 (PUA-43)
        ...
        // PUA-47: 위치 재추정 금지 — 저장된 parent_plan_index로 원본 tranche에 복원
        for child_plan in plans.iter().filter(|p| !p.released) {
            require!(
                child_plan.parent_plan_index != PARENT_PLAN_INDEX_UNSET,
                VestingError::InvalidParameters
            );
            let parent_idx = child_plan.parent_plan_index as usize;
            require!(
                parent_idx < parent_plan_chunk.plans.len(),
                VestingError::InvalidParameters
            );
            let parent_plan = &mut parent_plan_chunk.plans[parent_idx];
            require!(!parent_plan.released, VestingError::AlreadyReleased);
...
/// PUA-34: different-TGE **차감** 후보 (append / take_from_parent_plan).
/// 1) `!released` 필터
...
pub struct YearlyPlan {                             // Yearly plan unit
    ...
    /// PUA-47: different-TGE 차감 시 소비한 parent plan index (반환 identity)
    /// top-level / 미바인딩은 PARENT_PLAN_INDEX_UNSET
    pub parent_plan_index: u8,
}
...
/// PUA-50: Account 로드 시 data_len(신 space) 검사 **이후**에만 VestingPlanChunk deserialize.
/// 커스텀 constraint는 deserialize 이후라 구 계정 오역을 막지 못함.
#[derive(Clone)]
pub struct MigratedPlanChunk {
    pub inner: VestingPlanChunk,
}

impl std::ops::Deref for MigratedPlanChunk {
    type Target = VestingPlanChunk;
    fn deref(&self) -> &Self::Target {
        &self.inner
    }
}

impl std::ops::DerefMut for MigratedPlanChunk {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.inner
    }
}

impl anchor_lang::AccountSerialize for MigratedPlanChunk {
    fn try_serialize<W: std::io::Write>(&self, writer: &mut W) -> Result<()> {
        self.inner.try_serialize(writer)
    }
}

impl anchor_lang::AccountDeserialize for MigratedPlanChunk {
    fn try_deserialize(buf: &mut &[u8]) -> Result<Self> {
        require!(
            buf.len() >= PLAN_CHUNK_SPACE,
            VestingError::InvalidParameters
        );
        Ok(Self {
            inner: VestingPlanChunk::try_deserialize(buf)?,
        })
    }
    ...
    fn try_deserialize_unchecked(buf: &mut &[u8]) -> Result<Self> {
        ...
        Ok(Self {
            inner: VestingPlanChunk::try_deserialize_unchecked(buf)?,
        })
    }
}

impl Discriminator for MigratedPlanChunk {
    const DISCRIMINATOR: [u8; 8] = VestingPlanChunk::DISCRIMINATOR;
}

impl Owner for MigratedPlanChunk {
    fn owner() -> Pubkey {
        ID
    }
}
...
pub struct DoVesting<'info> {                       // do_vesting context definition
    ...
    pub plan_chunk: Account<'info, MigratedPlanChunk>,
...
pub struct UserCreateVesting<'info> {             // user_create_vesting context
    ...
    pub parent_plan_chunk: Box<Account<'info, MigratedPlanChunk>>,      // Parent plans
...
pub struct AppendYearlyPlan<'info> {               // append_yearly_plan context
    ...
    #[account(
        init_if_needed,
        payer = admin,
        space = PLAN_CHUNK_SPACE,                // Max 52 plans (size calculation in comments)
        seeds = [b"plans", vesting_account.key().as_ref()],
        bump
    )]
    pub plan_chunk: Account<'info, MigratedPlanChunk>,                  // Create/update the target plan chunk

    /// parent plan chunk — child append(deduct) 시 필수
    #[account(mut)]
    pub parent_plan_chunk: Option<Account<'info, MigratedPlanChunk>>,
...
// PUA-50: 구 VestingPlanChunk(17B YearlyPlan) → 신 레이아웃 마이그레이션
#[derive(Accounts)]
pub struct MigratePlanChunk<'info> {
    #[account(mut)]
    pub admin: Signer<'info>,
    ...
    /// CHECK: plans PDA — 구 레이아웃이라 Account로 로드하지 않음
    #[account(
        mut,
        seeds = [b"plans", vesting_account.key().as_ref()],
        bump
    )]
    pub plan_chunk: UncheckedAccount<'info>,

    /// child migrate 시 parent vesting (top-level은 생략)
    pub parent_vesting_account: Option<Account<'info, VestingAccount>>,

    /// CHECK: parent plans — child migrate 시 필수, 이미 신 레이아웃
    #[account(mut)]
    pub parent_plan_chunk: Option<UncheckedAccount<'info>>,

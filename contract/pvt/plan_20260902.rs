...
pub mod vesting {                                      // Start of the Anchor program module
    ...
    pub fn migrate_plan_chunk(ctx: Context<MigratePlanChunk>) -> Result<()> {
        require!(
            ctx.accounts.admin_config.admin == ctx.accounts.admin.key(),
            VestingError::Unauthorized
        );
        ...
        let (vesting_key, migrated_plans) = {
            let data = plan_chunk_info.try_borrow_data()?;
        ...
        if current_lamports < new_minimum {
            ...
            **admin_info.try_borrow_mut_lamports()? = admin_info
                ...
                .checked_sub(diff)
                .ok_or(VestingError::Overflow)?;
            **plan_chunk_info.try_borrow_mut_lamports()? = plan_chunk_info
                .lamports()
                .checked_add(diff)
                .ok_or(VestingError::Overflow)?;
        }
...
pub struct MigratePlanChunk<'info> {
    ...
    /// CHECK: plans PDA — 구 레이아웃이라 Account<VestingPlanChunk>로 로드하지 않음
    #[account(
        mut,
        seeds = [b"plans", vesting_account.key().as_ref()],
        bump

...
/// same-TGE + 빈 indices → release_time 매칭(정확).
/// different-TGE → 호출자가 제공한 index를 parent 후보/중복 규칙으로 검증 후 기록(추정 금지).
fn apply_parent_plan_indices_on_migrate(
    ...
    if same_tge && parent_plan_indices.is_empty() {
        for plan in plans.iter_mut() {
            ...
            let parent_idx = parent_plans
                ...
                .ok_or(VestingError::InvalidParameters)?;
            require!(
                !parent_plans[parent_idx].released,
                VestingError::AlreadyReleased
            );
    ...
    let unreleased: Vec<usize> = parent_plans
        .iter()
        .enumerate()
    ...
    // different-TGE 합법 슬롯 = 첫 unreleased(TGE) 제외 (amount 무관: 이미 소비된 0 슬롯 포함)
    let legal_diff_indices: Vec<usize> = unreleased.into_iter().skip(1).collect();
    ...
    for (plan, &idx) in plans.iter_mut().zip(parent_plan_indices.iter()) {
        ...
        if same_tge {
            ...
            require!(
                legal_diff_indices.contains(&parent_idx),
                VestingError::InvalidParameters
            );

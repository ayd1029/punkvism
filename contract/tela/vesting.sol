...
contract VestingContract is AccessControl, ReentrancyGuard {
    ...
    // --------- Plan Management (Solana append/update 로직 대응) ---------
    ...
    function updatePlanChunk(
        ...
        // 기존 plans index mapping은 완전 정리는 어렵지만, offchain에서 key별 plan 배열이 정답이므로
        // 실제 실행은 planIndexPlus1ByKey로만 lookup하는 방식을 유지한다면, 업데이트 전에 기존 매핑을 0으로 초기화해야 함.
        // 가스 고려로 여기서는 새 플랜의 releaseTime만 덮어씀
    ...
    function _deductFromParentPlans(
        ...
        if (tgeEqual) {
            ...
            // 자식의 unreleased 플랜을 순서대로 부모의 unreleased 플랜(1번부터)에 매칭해서 차감
